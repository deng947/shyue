...
// Node object
function Node(id, pid, name, ischeck, url, title, target, icon, iconOpen, open) {
	this.id = id;
	this.pid = pid;
...
dTree.prototype.add = function(id, pid, name, ischeck, url, title, target, icon, iconOpen, open) {
	this.aNodes[this.aNodes.length] = new Node(id, pid, name, ischeck, url, title, target, icon, iconOpen, open);
};
...
dTree.prototype.node = function(node, nodeId) {
	...
	if(this.config.useCheckBox && nodeId!=0){
		...
		str += '<input type="checkbox" name="checkId" id="c'+this.parEnum+'" onClick="caBox(\''+this.formId+'\',\'c'+this.parEnum+'\')" value="' + node.id  +'"' + ischeck + 'class="cx"/>';
	}
